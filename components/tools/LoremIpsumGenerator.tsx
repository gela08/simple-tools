...
export default function LoremIpsumGenerator() {
  ...
  const generate = () => {
    setOutput(Array.from({ length: paras }, makeParagraph).join("\n\n"));
    setCopied(false);
  };
    ...
    <div className="space-y-5">
      ...
      {output && (
        <div className="animate-in fade-in slide-in-from-top-2 duration-300">
          ...
          <div className="max-h-72 overflow-y-auto rounded-xl border border-border bg-surface-muted px-4 py-3 text-sm text-ink whitespace-pre-wrap shadow-inner leading-relaxed">
            {output}
          </div>
        </div>
      )}
